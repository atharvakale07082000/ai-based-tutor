...
description: Update the learner's Elo proficiency after a quiz and capture their mood from reflections.
allowed-tools: get_proficiency calculate_elo analyze_sentiment save_progress
---

# Progress tracking

Turn a quiz result + a reflection into an updated proficiency model.
...
4. Call `save_progress` with learner_id, topic, old_elo, new_elo, score, and mood to
   persist it.
5. Confirm warmly and specifically: the Elo change, XP earned, and one encouraging,
   honest note about where they're heading.
