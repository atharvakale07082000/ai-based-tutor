"""
Skill-gap analysis for the Job Tracker.
...
Both feed the ``jd_analyze`` pipeline behind ``/jobs/analyze/stream``.
"""
...
async def parse_jd(jd_text: str) -> dict:
    """Extract {company, role, seniority, required_skills} from a JD via the LLM."""
...
def _phrase_in(needle: str, haystack: str) -> bool:
    """Whole-word/phrase containment on word boundaries (avoids short-token false positives)."""
