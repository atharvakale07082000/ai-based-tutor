...
def _analysis_stream(
    jd_text: str, proficiency: dict, required_skills: list[str] | None
):
...
async def analyze_jd_stream(
    body: JDParseRequest, user_id: str = Depends(get_current_user_id)
):
...
async def update_job(
    job_id: str, body: JobUpdate, user_id: str = Depends(get_current_user_id)
):
    ...
    result = await col_job_applications().update_one(
        {"id": job_id, "learner_id": learner["id"]}, {"$set": updates}
    )
