...
def _record(eval_type: str, agent: str, score: float, passed: bool, details: dict, label: str = ""):
    _EVAL_RESULTS.append(
        {"eval_type": eval_type, "agent": agent, "score": score, "passed": passed, "details": details, "label": label}
    )
...
class TestCurriculumAgentIntegration:
    ...
    async def test_CA_01_generates_path_from_goals(self):
        ...
        _record("curriculum_ordering", "curriculum_agent", score, passed, details, "CA-01: goals→path ordering")
    ...
    async def test_CA_02_empty_goals_default_curriculum(self):
        ...
        score, passed, details = eval_curriculum_ordering({}, result)
        _record("curriculum_ordering", "curriculum_agent", score, passed, details, "CA-02: default curriculum ordering")
    ...
    async def test_CA_03_prioritises_low_elo_subtopics(self):
        ...
        score, passed, details = eval_curriculum_ordering({}, result)
        _record("curriculum_ordering", "curriculum_agent", score, passed, details, "CA-03: low-elo priority ordering")
...
class TestQuizAgentIntegration:
    ...
    async def test_QA_01_generates_five_questions_default(self):
        ...
        _record("quiz_format", "quiz_agent", score, passed, details, "QA-01: 5 questions at apply level")
    ...
    async def test_QA_02_low_elo_maps_to_remember(self):
        ...
        _record("quiz_format", "quiz_agent", score, passed, details, "QA-02: remember level at low Elo")
    ...
    async def test_QA_05_guardrail_filters_malformed_questions(self):
        ...
        _record("quiz_format", "quiz_agent", score, passed, details, "QA-05: guardrail filters 2/3 malformed questions")
...
class TestProgressAgentIntegration:
    ...
    async def test_PA_01_elo_increases_on_good_score(self):
        ...
        _record("progress_elo", "progress_agent", score, passed, details, "PA-01: good score → Elo increases")
    ...
    async def test_PA_02_elo_decreases_on_poor_score(self):
        ...
        _record("progress_elo", "progress_agent", score, passed, details, "PA-02: poor score → Elo decreases")
    ...
    async def test_PA_03_elo_clamped_at_zero(self):
        ...
        _record("progress_elo", "progress_agent", score, passed, details, "PA-03: Elo clamped at 0")
    ...
    async def test_PA_04_elo_clamped_at_1000(self):
        ...
        _record("progress_elo", "progress_agent", score, passed, details, "PA-04: Elo clamped at 1000")
...
class TestDoubtAgentIntegration:
    ...
    async def test_DA_01_answers_on_topic_question(self):
        ...
        _record("doubt_relevance", "doubt_agent", score, passed, details, "DA-01: on-topic Python question")
    ...
    async def test_DA_02_input_guardrail_blocks_injection(self):
        ...
        _record("guardrail_triggered", "doubt_agent", score, passed, details, "DA-02: injection attempt blocked")
    ...
    async def test_DA_03_empty_question_uses_fallback(self):
        ...
        with mock_all_tools(), patch("app.agents.doubt_agent.stream_doubt_response", side_effect=_mock_stream):
            from app.agents.doubt_agent import doubt_agent_node
    ...
    async def test_DA_04_grounding_warning_on_off_topic_response(self):
        ...
        with mock_all_tools(), patch("app.agents.doubt_agent.stream_doubt_response", side_effect=_off_topic_stream):
            from app.agents.doubt_agent import doubt_agent_node
...
class TestPlannerAgentIntegration:
    ...
    async def test_PL_01_no_curriculum_triggers_build(self):
        ...
        _record("planner_decision", "planner_agent", score, passed, details, "PL-01: no curriculum → trigger build")
    ...
    async def test_PL_02_existing_curriculum_routes_to_quiz(self):
        ...
        score, passed, details = eval_planner_decision(state, result)
        _record(
            "planner_decision", "planner_agent", score, passed, details, "PL-02: existing curriculum → quiz first topic"
        )
    ...
    async def test_PL_04_all_mastered_ends_session(self):
        ...
        score, passed, details = eval_planner_decision(state, result)
        _record("planner_decision", "planner_agent", score, passed, details, "PL-04: all mastered → end session")
    ...
    async def test_PL_05_max_iterations_ends_session(self):
        ...
        score, passed, details = eval_planner_decision(state, result)
        _record("planner_decision", "planner_agent", score, passed, details, "PL-05: iteration cap hit → end session")
...
class TestWorkflow_ColdStart:
    ...
    async def test_WF_CS_01_cold_start_full_chain(self):
        ...
        score, passed, details = eval_curriculum_ordering({}, result)
        _record(
            "curriculum_ordering", "orchestrator", score, passed, details, "WF-CS-01: cold start curriculum ordering"
        )

        # Eval quiz format
        from app.evals.evaluator import eval_quiz_format

        score2, passed2, details2 = eval_quiz_format({"current_topic": result["current_topic"]}, result)
        _record("quiz_format", "orchestrator", score2, passed2, details2, "WF-CS-01: cold start quiz format")
...
class TestWorkflow_Progress:
    ...
    async def test_WF_PR_01_good_score_advances_to_next_quiz(self):
        ...
        score, passed, details = eval_quiz_format({"current_topic": result["current_topic"]}, result)
        _record("quiz_format", "orchestrator", score, passed, details, "WF-PR-01: post-progress quiz format")
    ...
    async def test_WF_PR_02_last_topic_mastered_ends_session(self):
        ...
        _record(
            "planner_decision", "orchestrator", score, passed, details, "WF-PR-02: last topic mastered → session end"
        )
...
class TestWorkflow_DirectQuiz:
    ...
    async def test_WF_DQ_01_direct_quiz_ends_at_questions(self):
        ...
        score, passed, details = eval_quiz_format({"current_topic": "NumPy & Array Operations"}, result)
        _record("quiz_format", "orchestrator", score, passed, details, "WF-DQ-01: direct quiz format check")
...
class TestWorkflow_Doubt:
    ...
    async def test_WF_DO_01_doubt_answered_and_ends(self):
        ...
        _record(
            "doubt_relevance", "orchestrator", score, passed, details, "WF-DO-01: doubt on neural networks relevance"
        )
...
class TestWorkflow_FullSession:
    ...
    async def test_WF_FS_01_two_topic_session_completes(self):
        ...
        # ── Step 1: cold start ─────────────────────────────────────────────
        with mock_all_tools():
            step1 = await orchestrator.ainvoke(
                _base_state(
                    task_type="start",
        ...
        with mock_all_tools():
            step2 = await orchestrator.ainvoke(
                _base_state(
                    task_type="progress",
        ...
        if is_continuing:
            ...
            with mock_all_tools():
                step3 = await orchestrator.ainvoke(
                    _base_state(
                        task_type="progress",
...
def _print_eval_report(request):
    ...
    print("─" * 80)
    sum_header = f"{'Eval Type':<28} {'Agent':<22} {'Total':>6} {'Passed':>7} {'AvgScore':>9} {'PassRate':>9}"
    print(sum_header)
    print("─" * 80)
    ...
    for (etype, agent), records in sorted(agg.items()):
        ...
        print(f"{etype:<28} {agent:<22} {total:>6} {passed:>7} {avg:>9.3f} {rate:>8.0%}")

    print("─" * 80)
    overall_avg = sum(score_all) / len(score_all) if score_all else 0.0
    overall_rate = passed_all / total_all if total_all else 0.0
    print(f"{'OVERALL':<28} {'':<22} {total_all:>6} {passed_all:>7} {overall_avg:>9.3f} {overall_rate:>8.0%}")
    print("=" * 80)
